...
# Глобальный лок для предотвращения конфликтов FFMPEG
video_capture_lock = threading.Lock()

# ============================================
# МЕНЕДЖЕР ЯЗЫКОВ (LANGUAGE MANAGER)
# ============================================
class LanguageManager:
    # Единый словарь с English (используется и для создания файла, и как fallback)
    EN_TRANSLATIONS = {
        ...
        "optimize_threshold": "Optimization threshold (video count):",
        ...
        "hotkeys": "⌨️ Space:Pause/Resume | S:Stop | P:Pause | R:Resume | F:Fullscreen | H:Hide Panel | F1:Settings",
    ...
    def load_languages(self):
        """Загрузка языковых файлов из папки lang/"""
        # Исправленный путь: ищем папку lang относительно корня проекта
        lang_dir = self._find_lang_folder()
        ...
        for lang_file in lang_dir.glob('*.json'):
            ...
            try:
                with open(lang_file, 'r', encoding='utf-8') as f:
                    ...
                    self.language_names[lang_code] = self.translations[lang_code].get(
                        lang_name_key, lang_code.upper()
                    )
            except Exception as e:
                print(f"Error loading {lang_file}: {e}")
                self.translations[lang_code] = {}
                self.language_names[lang_code] = lang_code.upper()
        
        # Если файлы не найдены, используем встроенный English
        if not self.translations:
            self.translations = {'en': self.EN_TRANSLATIONS.copy()}
            self.language_names['en'] = 'English'
    
    def _find_lang_folder(self):
        """Поиск папки lang в различных возможных местах"""
        script_path = Path(__file__).resolve()
        
        # Варианты путей для поиска папки lang
        possible_paths = [
            script_path.parent.parent / 'lang',           # scr/ZVideoGridPlayer/../../lang
            script_path.parent / 'lang',                   # scr/ZVideoGridPlayer/lang
            Path.cwd() / 'lang',                           # текущая директория/lang
            Path.cwd().parent / 'lang',                    # текущая директория/../lang
        ]
        ...
        # Если не найдено, создаём в корневой директории проекта
        return script_path.parent.parent / 'lang'
    
    def _create_default_lang_files(self, lang_dir):
        """Создание файлов языков по умолчанию"""
        # Создаём только en.json из единой переменной
        with open(lang_dir / 'en.json', 'w', encoding='utf-8') as f:
            json.dump(self.EN_TRANSLATIONS, f, indent=4, ensure_ascii=False)
    
    def _get_builtin_translations(self):
        """Встроенные переводы (только English как fallback)"""
        return {'en': self.EN_TRANSLATIONS.copy()}
...
# ============================================
# ОСТАЛЬНЫЕ КЛАССЫ (SettingsPanel, VideoPlayerThread, ResizeHandle, CustomTitleBar, VideoGridPlayer)
# ============================================
# ... (остальной код из предыдущего полного файла)
...
class SettingsPanel:
    ...
    def create_content(self):
        ...
        self.title_label = tk.Label(header, text=lang_manager.get('settings_title'), 
                bg='#2c2c2c', fg='#ffffff',
    ...
    def create_settings_widgets(self, parent):
        ...
        # ==================== ЯЗЫК ИНТЕРФЕЙСА (ДИНАМИЧЕСКИ) ====================
        tk.Label(parent, text=lang_manager.get('language'), bg='#2c2c2c', fg='#ffffff',
        ...
        # Получаем доступные языки автоматически из LanguageManager
        lang_options = lang_manager.get_available_languages()
        
        lang_frame = tk.Frame(parent, bg='#2c2c2c')
        lang_frame.grid(row=row, column=0, sticky='w', padx=15, pady=5)
        
        for value, text in lang_options:
            rb = tk.Radiobutton(lang_frame, text=text, variable=self.language, value=value,
        ...
        # ==================== ВИДЕО БЭКЕНД ====================
        tk.Label(parent, text=lang_manager.get('video_backend'), bg='#2c2c2c', fg='#ffffff',
                font=('Segoe UI', 10, 'bold')).grid(row=row, column=0, sticky='w', padx=15, pady=(10,5))
        row += 1
        
        self.backend = tk.StringVar(value=self.settings.get('backend', 'AUTO'))
        backends = [
            ('AUTO', lang_manager.get('backend_auto')),
            ('FFMPEG', lang_manager.get('backend_ffmpeg')), 
            ('MSMF', lang_manager.get('backend_msmf')),
            ('DSHOW', lang_manager.get('backend_dshow'))
        ]
        ...
        # ==================== МАКС. КОЛИЧЕСТВО ВИДЕО ====================
        tk.Label(parent, text=lang_manager.get('max_videos'), bg='#2c2c2c', fg='#ffffff',
                font=('Segoe UI', 10, 'bold')).grid(row=row, column=0, sticky='w', padx=15, pady=(10,5))
        row += 1
        ...
        # ==================== МАКСИМАЛЬНЫЙ FPS ====================
        tk.Label(parent, text=lang_manager.get('max_fps'), bg='#2c2c2c', fg='#ffffff',
                font=('Segoe UI', 10, 'bold')).grid(row=row, column=0, sticky='w', padx=15, pady=(10,5))
        row += 1
        ...
        # ==================== МИНИМАЛЬНЫЙ РАЗМЕР ВИДЕО ====================
        tk.Label(parent, text=lang_manager.get('min_video_size'), bg='#2c2c2c', fg='#ffffff',
                font=('Segoe UI', 10, 'bold')).grid(row=row, column=0, sticky='w', padx=15, pady=(10,5))
        row += 1
        ...
        tk.Label(parent, text=lang_manager.get('resize_quality'), bg='#2c2c2c', fg='#ffffff',
                font=('Segoe UI', 10, 'bold')).grid(row=row, column=0, sticky='w', padx=15, pady=(10,5))
        row += 1
        
        self.resize_quality = tk.StringVar(value=self.settings.get('resize_quality', lang_manager.get('quality_medium')))
        quality_combo = ttk.Combobox(parent, textvariable=self.resize_quality,
                                     values=[lang_manager.get('quality_low'), 
                                            lang_manager.get('quality_medium'), 
                                            lang_manager.get('quality_high')],
        ...
        tk.Label(parent, text=lang_manager.get('queue_size'), bg='#2c2c2c', fg='#ffffff',
                font=('Segoe UI', 10, 'bold')).grid(row=row, column=0, sticky='w', padx=15, pady=(10,5))
        row += 1
        ...
        # ==================== ПОРОГ ОПТИМИЗАЦИИ ====================
        tk.Label(parent, text=lang_manager.get('optimize_threshold'), bg='#2c2c2c', fg='#ffffff',
                font=('Segoe UI', 10, 'bold')).grid(row=row, column=0, sticky='w', padx=15, pady=(10,5))
        row += 1
        ...
        tk.Checkbutton(parent, text=lang_manager.get('skip_frames'),
        ...
        # ==================== ЦВЕТОВАЯ ТЕМА ====================
        tk.Label(parent, text=lang_manager.get('theme'), bg='#2c2c2c', fg='#ffffff',
                font=('Segoe UI', 10, 'bold')).grid(row=row, column=0, sticky='w', padx=15, pady=(10,5))
        row += 1
        
        self.theme = tk.StringVar(value=self.settings.get('theme', 'dark'))
        theme_combo = ttk.Combobox(parent, textvariable=self.theme, 
                                   values=['dark', 'light'],
                                   state='readonly', width=25)
        ...
        # ==================== ПРОЗРАЧНОСТЬ ОКНА ====================
        tk.Label(parent, text=lang_manager.get('opacity'), bg='#2c2c2c', fg='#ffffff',
                font=('Segoe UI', 10, 'bold')).grid(row=row, column=0, sticky='w', padx=15, pady=(10,5))
        row += 1
        ...
        # ==================== ЗАДЕРЖКА МЕЖДУ ОТКРЫТИЕМ ВИДЕО ====================
        tk.Label(parent, text=lang_manager.get('open_delay'), bg='#2c2c2c', fg='#ffffff',
                font=('Segoe UI', 10, 'bold')).grid(row=row, column=0, sticky='w', padx=15, pady=(10,5))
        row += 1
        
        self.open_delay = tk.IntVar(value=self.settings.get('open_delay', 50))
        delay_frame = tk.Frame(parent, bg='#2c2c2c')
        delay_frame.grid(row=row, column=0, sticky='ew', padx=15, pady=5)
        ttk.Scale(delay_frame, from_=0, to=500, orient=tk.HORIZONTAL, variable=self.open_delay, length=250).pack(side=tk.LEFT)
        self.delay_label = tk.Label(delay_frame, text=str(self.open_delay.get()), bg='#2c2c2c', fg='#0078d4', width=4)
        self.delay_label.pack(side=tk.LEFT, padx=10)
        self.open_delay.trace('w', lambda *a: self.delay_label.config(text=str(self.open_delay.get())))
        row += 1
        ...
        # ==================== КНОПКА СОХРАНЕНИЯ ====================
        save_btn = tk.Button(parent, text=lang_manager.get('apply_settings'), command=self.save_settings,
    ...
    def save_settings(self):
        ...
        self.settings.update({
            ...
            'open_delay': self.open_delay.get(),
        })
        
        # Применяем язык
        lang_manager.set_language(self.settings.get('language', 'en'))
        ...
        self.show_notification(lang_manager.get('settings_saved'))
    ...
    def show(self):
        if not self.is_visible:
            ...
            self.panel.lift()
            # Пересоздаём виджеты с текущим языком
            self.create_settings_widgets(self.scrollable_frame)
...
class VideoPlayerThread:
    def __init__(self, video_data, fps_control, optimize_flag, settings, video_index):
    ...
    def _play_loop(self):
        try:
            open_delay = self.settings.get('open_delay', 50)
            time.sleep(self.video_index * open_delay / 1000.0)
            
            self.cap = self._get_video_capture(self.video_data['path'])
            if not self.cap or not self.cap.isOpened():
                self.video_data['valid'] = False
                return
            ...
            if width > 0 and height > 0:
                ...
                self.video_data['aspect_ratio'] = self.video_aspect_ratio
            else:
            ...
            while self.is_playing:
                try:
                    ...
                    quality = self.settings.get('resize_quality', 'Medium')
                    if quality == 'Low (fast)' or quality == 'Низкое (быстро)':
                        interpolation = cv2.INTER_NEAREST
                    elif quality == 'High (slow)' or quality == 'Высокое (медленно)':
                    ...
                    h, w = frame.shape[:2]
                    video_ratio = w / h if h > 0 else 16/9
                    cell_ratio = cell_w / cell_h if cell_h > 0 else 16/9
                    ...
                    if x_offset >= 0 and y_offset >= 0:
                        end_x = min(x_offset + new_w, cell_w)
                        end_y = min(y_offset + new_h, cell_h)
...
class ResizeHandle:
    ...
    def __init__(self, root, app):
        self.root = root
        self.app = app
        self.resize_size = 10  # Вернул к оригинальному размеру (было 20)
        self.resizing = False
    ...
    def create_handles(self):
        ...
        for name, relx, rely, anchor, cursor in corners:
            ...
            handle.bind('<Enter>', lambda e, h=handle: h.configure(bg='#2c2c2c'))
            handle.bind('<Leave>', lambda e, h=handle: h.configure(bg='#1c1c1c'))
            self.handles[name] = handle
        
        # Края
        edges = [
            ('e', 1.0, 0.5, 'e', 'size_we', s, 4),
            ('w', 0.0, 0.5, 'w', 'size_we', s, 4),
            ('s', 0.5, 1.0, 's', 'size_ns', 4, s),
            ('n', 0.5, 0.0, 'n', 'size_ns', 4, s),
        ]
    ...
    def on_resize(self, event):
        ...
        if self.resize_direction == 'se':
            ...
            new_w = max(min_w, self.start_w - dx)
            new_x = self.start_root_x + (self.start_w - new_w)
            new_h = max(min_h, self.start_h + dy)
        elif self.resize_direction == 'ne':
            new_w = max(min_w, self.start_w + dx)
            new_h = max(min_h, self.start_h - dy)
            new_y = self.start_root_y + (self.start_h - new_h)
    ...
    def update_handles(self):
        """Обновить позиции ручек после изменения размера окна"""
        self.create_handles()
...
class CustomTitleBar:
    ...
    def toggle_maximize(self, event=None):
        ...
        self.app.resize_handler.update_handles()
...
class VideoGridPlayer:
    def __init__(self, root):
        ...
        self.title_bar = CustomTitleBar(root, self, lang_manager.get('app_title'))
    ...
    def update_ui_texts(self):
        ...
        if hasattr(self, 'load_btn'):
            self.load_btn.configure(text=lang_manager.get('select_folder'))
        if hasattr(self, 'start_btn'):
            self.start_btn.configure(text=lang_manager.get('start'))
        if hasattr(self, 'pause_btn'):
            self.pause_btn.configure(text=lang_manager.get('pause'))
        if hasattr(self, 'resume_btn'):
            self.resume_btn.configure(text=lang_manager.get('resume'))
        if hasattr(self, 'stop_btn'):
            self.stop_btn.configure(text=lang_manager.get('stop'))
        if hasattr(self, 'settings_btn'):
            self.settings_btn.configure(text=lang_manager.get('settings'))
        if hasattr(self, 'hide_btn'):
            self.hide_btn.configure(text=lang_manager.get('hide_panel'))
        if hasattr(self, 'info_label'):
            self.info_label.configure(text=lang_manager.get('no_videos'))
        if hasattr(self, 'show_names_check'):
            self.show_names_check.configure(text=lang_manager.get('show_names'))
        if hasattr(self, 'optimize_check'):
            self.optimize_check.configure(text=lang_manager.get('optimization'))
        if hasattr(self, 'hotkeys_label'):
            self.hotkeys_label.configure(text=lang_manager.get('hotkeys'))
        ...
        for widget in self.control_panel.winfo_children():
            for child in widget.winfo_children():
                if isinstance(child, tk.Label):
                    ...
                    if 'FPS' in text:
                        child.configure(text=lang_manager.get('fps_label'))
                    elif 'Колонок' in text or 'Columns' in text:
                        child.configure(text=lang_manager.get('cols_label'))
        
        if self.videos:
            self.info_label.configure(text=lang_manager.get('videos_info', len(self.videos), self.cols))
    ...
    def create_control_panel_content(self, parent):
        ...
        self.load_btn = tk.Button(button_frame, text=lang_manager.get('select_folder'), command=self.load_folder, **btn_style)
        self.load_btn.pack(side=tk.LEFT, padx=4)
        
        self.start_btn = tk.Button(button_frame, text=lang_manager.get('start'), command=self.start_playback, **btn_style)
        self.start_btn.pack(side=tk.LEFT, padx=4)
        
        self.pause_btn = tk.Button(button_frame, text=lang_manager.get('pause'), command=self.pause_playback_all, **btn_style)
        self.pause_btn.pack(side=tk.LEFT, padx=4)
        
        self.resume_btn = tk.Button(button_frame, text=lang_manager.get('resume'), command=self.resume_playback_all, **btn_style)
        self.resume_btn.pack(side=tk.LEFT, padx=4)
        
        self.stop_btn = tk.Button(button_frame, text=lang_manager.get('stop'), command=self.stop_playback, **btn_style)
        self.stop_btn.pack(side=tk.LEFT, padx=4)
        
        self.settings_btn = tk.Button(button_frame, text=lang_manager.get('settings'), command=self.settings_panel.toggle, **btn_style)
        self.settings_btn.pack(side=tk.LEFT, padx=4)
        
        self.hide_btn = tk.Button(button_frame, text=lang_manager.get('hide_panel'), command=self.toggle_panel, **btn_style)
        self.hide_btn.pack(side=tk.LEFT, padx=4)
        
        self.info_label = tk.Label(button_frame, text=lang_manager.get('no_videos'), bg='#1c1c1c', fg='#8c8c8c', font=('Segoe UI', 10))
        ...
        tk.Label(fps_frame, text=lang_manager.get('fps_label'), bg='#1c1c1c', fg='#ffffff', font=('Segoe UI', 10)).pack(side=tk.LEFT, padx=4)
        ...
        tk.Label(cols_frame, text=lang_manager.get('cols_label'), bg='#1c1c1c', fg='#ffffff', font=('Segoe UI', 10)).pack(side=tk.LEFT, padx=4)
        ...
        apply_btn = tk.Button(cols_frame, text=lang_manager.get('apply'), command=self.apply_grid_size, **btn_style)
        apply_btn.pack(side=tk.LEFT, padx=8)
        
        self.show_names_var = tk.BooleanVar(value=False)
        self.show_names_check = tk.Checkbutton(quick_frame, text=lang_manager.get('show_names'), variable=self.show_names_var,
        ...
        self.optimize_check = tk.Checkbutton(quick_frame, text=lang_manager.get('optimization'), variable=self.optimize_var,
        ...
        self.hotkeys_label = tk.Label(quick_frame, text=lang_manager.get('hotkeys'),
    ...
    def rearrange_grid(self, keep_playing=False):
        ...
        col_heights = [0] * cols
        col_videos = [[] for _ in range(cols)]
        ...
        for i, video_data in enumerate(current_videos):
            ...
            video_height = max(min_size // 2, video_height)
            
            min_col = col_heights.index(min(col_heights))
            col_videos[min_col].append((video_data, video_height))
            col_heights[min_col] += video_height + 6
        ...
        for col_idx, col_items in enumerate(col_videos):
            ...
            for video_data, video_height in col_items:
                ...
                frame.place(x=col_idx * (video_width + 6), y=y_offset, width=video_width, height=video_height)
                ...
                video_canvas.pack(padx=2, pady=2, expand=True, fill=tk.BOTH)
                ...
                y_offset += video_height + 6
        
        max_height = max(col_heights) if col_heights else 0
        self.scrollable_frame.configure(width=cols * (video_width + 6), height=max_height)
        
        self.info_label.config(text=lang_manager.get('videos_info', len(self.videos), self.cols))
    ...
    def load_folder(self):
        ...
        if not unique_videos:
            messagebox.showwarning("Warning", lang_manager.get('warning_no_videos'))
            return
            ...
            try:
                with video_capture_lock:
                    ...
                    if cap.isOpened():
                        ...
                        if w > 0 and h > 0:
                            video_ratios[str(video_path)] = w / h
                    cap.release()
                time.sleep(0.02)
            except:
                video_ratios[str(video_path)] = 16/9
        
        col_heights = [0] * self.cols
        col_videos = [[] for _ in range(self.cols)]
        
        for video_path in unique_videos:
            aspect_ratio = video_ratios.get(str(video_path), 16/9)
            if aspect_ratio <= 0:
                aspect_ratio = 16/9
            video_height = int(video_width / aspect_ratio)
            video_height = max(min_size // 2, video_height)
        ...
        max_height = max(col_heights) if col_heights else 0
        self.scrollable_frame.configure(width=self.cols * (video_width + 6), height=max_height)
    ...
    def start_playback(self):
        if not self.videos:
            messagebox.showwarning("Warning", lang_manager.get('warning_load_first'))
            return
        if self.is_playing:
            return
        ...
        for idx, video in enumerate(self.videos):
            thread = VideoPlayerThread(video, lambda: self.fps_var.get(), lambda: self.optimize_var.get(), 
                                       self.settings, idx)
            thread.start()
            video['valid'] = True
        ...
        if not self.video_threads:
            messagebox.showerror("Error", lang_manager.get('error_open_video'))
            return
