...
class CustomTitleBar:
    ...
    def setup_resize_handles(self):
        """Создание областей для растягивания окна"""
        s = self.resize_size
        
        # Углы
        self.corner_se = tk.Frame(self.root, bg='#1c1c1c', cursor='size_nw_se')
        self.corner_se.place(relx=1.0, rely=1.0, anchor='se', width=s, height=s)
        self.corner_se.bind('<ButtonPress-1>', lambda e: self.start_resize_corner('se'))
        self.corner_se.bind('<B1-Motion>', self.resize_corner)
        self.corner_se.bind('<ButtonRelease-1>', self.stop_resize)
        
        self.corner_sw = tk.Frame(self.root, bg='#1c1c1c', cursor='size_ne_sw')
        self.corner_sw.place(relx=0.0, rely=1.0, anchor='sw', width=s, height=s)
        self.corner_sw.bind('<ButtonPress-1>', lambda e: self.start_resize_corner('sw'))
        self.corner_sw.bind('<B1-Motion>', self.resize_corner)
        self.corner_sw.bind('<ButtonRelease-1>', self.stop_resize)
        
        self.corner_ne = tk.Frame(self.root, bg='#1c1c1c', cursor='size_ne_sw')
        self.corner_ne.place(relx=1.0, rely=0.0, anchor='ne', width=s, height=s)
        self.corner_ne.bind('<ButtonPress-1>', lambda e: self.start_resize_corner('ne'))
        self.corner_ne.bind('<B1-Motion>', self.resize_corner)
        self.corner_ne.bind('<ButtonRelease-1>', self.stop_resize)
        
        self.corner_nw = tk.Frame(self.root, bg='#1c1c1c', cursor='size_nw_se')
        self.corner_nw.place(relx=0.0, rely=0.0, anchor='nw', width=s, height=s)
        self.corner_nw.bind('<ButtonPress-1>', lambda e: self.start_resize_corner('nw'))
        self.corner_nw.bind('<B1-Motion>', self.resize_corner)
        self.corner_nw.bind('<ButtonRelease-1>', self.stop_resize)
        
        # Края
        self.edge_e = tk.Frame(self.root, bg='#1c1c1c', cursor='size_we')
        self.edge_e.place(relx=1.0, rely=0.5, anchor='e', width=s, height=1)
        self.edge_e.bind('<ButtonPress-1>', lambda e: self.start_resize_edge('e'))
        self.edge_e.bind('<B1-Motion>', self.resize_edge)
        self.edge_e.bind('<ButtonRelease-1>', self.stop_resize)
        
        self.edge_w = tk.Frame(self.root, bg='#1c1c1c', cursor='size_we')
        self.edge_w.place(relx=0.0, rely=0.5, anchor='w', width=s, height=1)
        self.edge_w.bind('<ButtonPress-1>', lambda e: self.start_resize_edge('w'))
        self.edge_w.bind('<B1-Motion>', self.resize_edge)
        self.edge_w.bind('<ButtonRelease-1>', self.stop_resize)
        
        self.edge_s = tk.Frame(self.root, bg='#1c1c1c', cursor='size_ns')
        self.edge_s.place(relx=0.5, rely=1.0, anchor='s', width=1, height=s)
        self.edge_s.bind('<ButtonPress-1>', lambda e: self.start_resize_edge('s'))
        self.edge_s.bind('<B1-Motion>', self.resize_edge)
        self.edge_s.bind('<ButtonRelease-1>', self.stop_resize)
        
        self.edge_n = tk.Frame(self.root, bg='#1c1c1c', cursor='size_ns')
        self.edge_n.place(relx=0.5, rely=0.0, anchor='n', width=1, height=s)
        self.edge_n.bind('<ButtonPress-1>', lambda e: self.start_resize_edge('n'))
        self.edge_n.bind('<B1-Motion>', self.resize_edge)
        self.edge_n.bind('<ButtonRelease-1>', self.stop_resize)
        
    def start_resize_corner(self, corner):
        self.resize_corner_type = corner
        self.resize_start = (self.root.winfo_x(), self.root.winfo_y(), 
                            self.root.winfo_width(), self.root.winfo_height())
        self.app.pause_playback()
        
    def start_resize_edge(self, edge):
        self.resize_edge_type = edge
        self.resize_start = (self.root.winfo_x(), self.root.winfo_y(), 
                            self.root.winfo_width(), self.root.winfo_height())
        self.app.pause_playback()
    ...
    def resize_corner(self, event):
        ...
        if self.resize_corner_type == 'se':
            new_w = max(800, mx - x)
            new_h = max(600, my - y)
            self.root.geometry(f"{new_w}x{new_h}")
        elif self.resize_corner_type == 'sw':
            new_w = max(800, x + w - mx)
            new_x = mx
            new_h = max(600, my - y)
            self.root.geometry(f"{new_w}x{new_h}+{new_x}+{y}")
        elif self.resize_corner_type == 'ne':
            new_w = max(800, mx - x)
            new_h = max(600, y + h - my)
            self.root.geometry(f"{new_w}x{new_h}+{x}+{my}")
        elif self.resize_corner_type == 'nw':
            new_w = max(800, x + w - mx)
            new_x = mx
            new_h = max(600, y + h - my)
            self.root.geometry(f"{new_w}x{new_h}+{new_x}+{my}")
        
        self.app.update_grid_size()
        
    def resize_edge(self, event):
        if not self.resize_start:
            return
        x, y, w, h = self.resize_start
        mx = self.root.winfo_pointerx()
        ...
        if self.resize_edge_type == 'e':
            new_w = max(800, mx - x)
            self.root.geometry(f"{new_w}x{h}")
        elif self.resize_edge_type == 'w':
            new_w = max(800, x + w - mx)
            new_x = mx
            self.root.geometry(f"{new_w}x{h}+{new_x}+{y}")
        elif self.resize_edge_type == 's':
            new_h = max(600, my - y)
            self.root.geometry(f"{w}x{new_h}")
        elif self.resize_edge_type == 'n':
            new_h = max(600, y + h - my)
            self.root.geometry(f"{w}x{new_h}+{x}+{my}")
