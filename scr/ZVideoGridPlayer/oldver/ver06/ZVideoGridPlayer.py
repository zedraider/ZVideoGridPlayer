...
class VideoGridPlayer:
    ...
    def on_window_resize(self, event):
        ...
        if event.widget == self.root:
            # Откладываем обновление сетки, чтобы избежать множественных вызовов
            if self.after_id:
    ...
    def rearrange_grid(self, keep_playing=False):
        ...
        canvas_width = self.canvas.winfo_width() - 20  # отступы для скролла
        if canvas_width < 100:
            canvas_width = 1200
        
        # Автоматический расчет размера видео в зависимости от количества колонок
        video_width = max(150, (canvas_width - (cols * 6)) // cols)  # 6 пикселей на отступы
