...
os.environ['OPENCV_FFMPEG_LOG_LEVEL'] = '-8'  # Отключаем FFMPEG логи
...
class VideoGridPlayer:
    def __init__(self, root):
        ...
        # Доступные бэкенды с понятными названиями (только стабильные)
        self.backend_options = {
            'CAP_ANY': 'Авто (рекомендуется)',
            'CAP_FFMPEG': 'FFMPEG (стабильный)',
        }
    ...
    def create_control_panel(self, parent):
        ...
        self.backend_combo.set(self.backend_options['CAP_ANY'])
    ...
    def get_video_capture(self, path):
        ...
        try:
            # Используем только FFMPEG и AUTO для стабильности
            if backend_id == 'CAP_FFMPEG':
                backend_value = getattr(cv2, 'CAP_FFMPEG', None)
                if backend_value is not None:
                    cap = cv2.VideoCapture(path, backend_value)
                    if cap.isOpened():
                        return cap
            # По умолчанию используем AUTO
...
def main():
    # Отключаем вывод предупреждений OpenCV
    import warnings
