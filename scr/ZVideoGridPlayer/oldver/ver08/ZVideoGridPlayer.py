...
class CustomTitleBar:
    ...
    def __init__(self, root, app, title="ZVideoGridPlayer"):
        ...
        # Кнопки управления (правильный порядок: закрыть, развернуть, свернуть)
        button_bg = '#1c1c1c'
    ...
    def ensure_custom_titlebar(self):
        ...
        try:
            if self.root.winfo_viewable():
                # Убеждаемся, что override-redirect установлен
    ...
    def minimize_window(self, event):
        ...
        try:
            # Временно отключаем override-redirect
    ...
    def toggle_maximize(self, event):
        ...
        # Убеждаемся, что кастомный заголовок на месте
...
class VideoGridPlayer:
    def __init__(self, root):
        ...
        # Доступные кодеки/бэкенды
        self.available_backends = self.get_available_backends()
        
        # Текущий выбранный бэкенд
        self.current_backend = tk.StringVar(value=self.available_backends[0] if self.available_backends else "AUTO")
        ...
        self.max_fps = 30  # Ограничение FPS для производительности
    ...
    def get_available_backends(self):
        """Получение доступных видео бэкендов"""
        backends = []
        
        # Проверяем доступные бэкенды
        try:
            # Получаем список всех доступных бэкендов
            available = []
            for i in range(10):
                backend_name = cv2.videoio_registry.getBackendName(i)
                if backend_name:
                    available.append(backend_name)
            
            # Приоритетные бэкенды для производительности
            preferred = ['FFMPEG', 'MSMF', 'DSHOW', 'CV_IMAGES', 'ANY']
            
            # Добавляем доступные бэкенды в порядке приоритета
            for pref in preferred:
                if pref in available:
                    backends.append(pref)
            ...
            # Если не удалось получить список, используем стандартные
            backends = ['AUTO', 'FFMPEG', 'MSMF', 'DSHOW']
        
        return backends if backends else ['AUTO']
    ...
    def create_control_panel(self, parent):
        ...
        # Чекбокс для ограничения FPS
        self.limit_fps_var = tk.BooleanVar(value=True)
        self.limit_fps_check = tk.Checkbutton(settings_frame, 
                                             text="⚡ Оптимизация", 
                                             variable=self.limit_fps_var,
                                             bg='#1c1c1c', fg='#ffffff',
                                             selectcolor='#1c1c1c',
                                             activebackground='#1c1c1c',
                                             font=('Segoe UI', 10))
        self.limit_fps_check.pack(side=tk.LEFT, padx=10)
    ...
    def rearrange_grid(self, keep_playing=False):
        ...
        self.info_label.config(text=f"📹 {len(self.videos)} | {self.cols} колонок | {self.current_backend.get()}")
        
        if was_playing:
            self.start_playback()
    
    def get_video_capture(self, path):
        """Создание VideoCapture с выбранным бэкендом"""
        ...
        try:
            if backend == "AUTO":
                ...
                # Пытаемся использовать указанный бэкенд
                backend_id = -1
                for i in range(10):
                    if cv2.videoio_registry.getBackendName(i) == backend:
                        backend_id = i
                        break
                
                if backend_id != -1:
                    return cv2.VideoCapture(path, backend_id)
                else:
                    return cv2.VideoCapture(path)
        except:
            return cv2.VideoCapture(path)
    ...
    def _play_loop(self):
        """Цикл воспроизведения с оптимизацией"""
        target_fps = self.fps_var.get()
        
        # Ограничиваем FPS для производительности при большом количестве видео
        if self.limit_fps_var.get() and len(self.videos) > 16:
        ...
        while self.is_playing:
            ...
            for idx, video in enumerate(self.videos):
                ...
                try:
                    ...
                    # Оптимизация: пропускаем кадры если не успеваем
                    if len(self.videos) > 24 and frame_counter % 2 == 0:
            ...
            if self.limit_fps_var.get() and len(self.videos) > 20:
                current_fps = 1.0 / (time.time() - last_time)
                if current_fps < target_fps * 0.8:
                    # Если не успеваем, снижаем целевой FPS
                    new_fps = max(15, target_fps - 2)
            ...
            frame_time = 1.0 / interface_fps if interface_fps > 0 else 0.033
