...
class SettingsPanel:
    ...
    def create_content(self):
        ...
        # Прокрутка мышкой для панели настроек
        def on_mousewheel(event):
            canvas.yview_scroll(int(-1*(event.delta/120)), "units")
        canvas.bind("<MouseWheel>", on_mousewheel)
        
        # Привязываем также к scrollable_frame для надежности
...
class CustomTitleBar:
    ...
    def setup_resize_handles(self):
        ...
        resize_size = 10  # Увеличиваем зону захвата для удобства
        
        # Нижний правый угол
        self.bottom_right = tk.Frame(self.root, bg='#1c1c1c', cursor='size_nw_se')
        self.bottom_right.place(relx=1.0, rely=1.0, anchor='se', width=resize_size, height=resize_size)
        self.bottom_right.bind('<Enter>', lambda e: self.bottom_right.configure(bg='#3c3c3c'))
        self.bottom_right.bind('<Leave>', lambda e: self.bottom_right.configure(bg='#1c1c1c'))
        ...
        self.bottom_left.place(relx=0.0, rely=1.0, anchor='sw', width=resize_size, height=resize_size)
        self.bottom_left.bind('<Enter>', lambda e: self.bottom_left.configure(bg='#3c3c3c'))
        self.bottom_left.bind('<Leave>', lambda e: self.bottom_left.configure(bg='#1c1c1c'))
        ...
        # Верхний правый угол
        self.top_right = tk.Frame(self.root, bg='#1c1c1c', cursor='size_ne_sw')
        self.top_right.place(relx=1.0, rely=0.0, anchor='ne', width=resize_size, height=resize_size)
        self.top_right.bind('<Enter>', lambda e: self.top_right.configure(bg='#3c3c3c'))
        self.top_right.bind('<Leave>', lambda e: self.top_right.configure(bg='#1c1c1c'))
        self.top_right.bind('<ButtonPress-1>', self.start_resize)
        self.top_right.bind('<B1-Motion>', self.resize_top_right)
        self.top_right.bind('<ButtonRelease-1>', self.stop_resize)
        
        # Верхний левый угол
        self.top_left = tk.Frame(self.root, bg='#1c1c1c', cursor='size_nw_se')
        self.top_left.place(relx=0.0, rely=0.0, anchor='nw', width=resize_size, height=resize_size)
        self.top_left.bind('<Enter>', lambda e: self.top_left.configure(bg='#3c3c3c'))
        self.top_left.bind('<Leave>', lambda e: self.top_left.configure(bg='#1c1c1c'))
        self.top_left.bind('<ButtonPress-1>', self.start_resize)
        self.top_left.bind('<B1-Motion>', self.resize_top_left)
        self.top_left.bind('<ButtonRelease-1>', self.stop_resize)
        ...
        self.right_edge.bind('<Enter>', lambda e: self.right_edge.configure(bg='#3c3c3c'))
        self.right_edge.bind('<Leave>', lambda e: self.right_edge.configure(bg='#1c1c1c'))
        ...
        # Левый край
        self.left_edge = tk.Frame(self.root, bg='#1c1c1c', cursor='size_we')
        self.left_edge.place(relx=0.0, rely=0.5, anchor='w', width=resize_size, height=1)
        self.left_edge.bind('<Enter>', lambda e: self.left_edge.configure(bg='#3c3c3c'))
        self.left_edge.bind('<Leave>', lambda e: self.left_edge.configure(bg='#1c1c1c'))
        self.left_edge.bind('<ButtonPress-1>', self.start_resize)
        self.left_edge.bind('<B1-Motion>', self.resize_left)
        self.left_edge.bind('<ButtonRelease-1>', self.stop_resize)
        ...
        self.bottom_edge.bind('<Enter>', lambda e: self.bottom_edge.configure(bg='#3c3c3c'))
        self.bottom_edge.bind('<Leave>', lambda e: self.bottom_edge.configure(bg='#1c1c1c'))
        ...
        # Верхний край
        self.top_edge = tk.Frame(self.root, bg='#1c1c1c', cursor='size_ns')
        self.top_edge.place(relx=0.5, rely=0.0, anchor='n', width=1, height=resize_size)
        self.top_edge.bind('<Enter>', lambda e: self.top_edge.configure(bg='#3c3c3c'))
        self.top_edge.bind('<Leave>', lambda e: self.top_edge.configure(bg='#1c1c1c'))
        self.top_edge.bind('<ButtonPress-1>', self.start_resize)
        self.top_edge.bind('<B1-Motion>', self.resize_top)
        self.top_edge.bind('<ButtonRelease-1>', self.stop_resize)
    ...
    def resize_top_right(self, event):
        x = self.root.winfo_pointerx()
        y = self.root.winfo_pointery()
        root_x = self.root.winfo_rootx()
        root_y = self.root.winfo_rooty()
        width = max(800, x - root_x)
        height = self.root.winfo_height() + (root_y - y)
        if height > 600:
            self.root.geometry(f"{width}x{height}+{root_x}+{y}")
            self.app.update_grid_size()
            
    def resize_top_left(self, event):
        x = self.root.winfo_pointerx()
        y = self.root.winfo_pointery()
        root_x = self.root.winfo_rootx()
        root_y = self.root.winfo_rooty()
