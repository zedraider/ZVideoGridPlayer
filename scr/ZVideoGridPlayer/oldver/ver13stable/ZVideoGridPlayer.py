...
class SettingsPanel:
    ...
    def create_settings_widgets(self, parent):
        ...
        backends = [
            ...
            ('FFMPEG', 'FFMPEG (универсальный)'),
            ('MSMF', 'Microsoft Media Foundation'),
            ('DSHOW', 'DirectShow (классический)')
        ]
        ...
        ttk.Scale(fps_frame, from_=1, to=120, orient=tk.HORIZONTAL,
                 variable=self.fps_limit, length=250).pack(side=tk.LEFT)
        self.fps_limit_label = tk.Label(fps_frame, text=str(self.fps_limit.get()), 
                                       bg='#2c2c2c', fg='#0078d4', width=4)
        ...
        ttk.Scale(size_frame, from_=80, to=300, orient=tk.HORIZONTAL,
                 variable=self.min_video_size, length=250).pack(side=tk.LEFT)
        self.min_size_label = tk.Label(size_frame, text=str(self.min_video_size.get()), 
                                      bg='#2c2c2c', fg='#0078d4', width=4)
        ...
        ttk.Scale(queue_frame, from_=1, to=5, orient=tk.HORIZONTAL,
                 variable=self.queue_size, length=250).pack(side=tk.LEFT)
        self.queue_label = tk.Label(queue_frame, text=str(self.queue_size.get()), 
                                   bg='#2c2c2c', fg='#0078d4', width=4)
        ...
        ttk.Scale(threshold_frame, from_=4, to=32, orient=tk.HORIZONTAL,
                 variable=self.optimize_threshold, length=250).pack(side=tk.LEFT)
        self.threshold_label = tk.Label(threshold_frame, text=str(self.optimize_threshold.get()), 
                                       bg='#2c2c2c', fg='#0078d4', width=4)
        ...
        ttk.Scale(opacity_frame, from_=0.5, to=1.0, orient=tk.HORIZONTAL,
                 variable=self.opacity, length=250).pack(side=tk.LEFT)
        self.opacity_label = tk.Label(opacity_frame, text=f"{self.opacity.get():.1f}", 
                                     bg='#2c2c2c', fg='#0078d4', width=4)
        ...
        save_btn = tk.Button(parent, text="💾 Применить настройки", 
    ...
    def save_settings(self):
        ...
        # Если изменились важные настройки, перезапускаем воспроизведение
    ...
    def show(self):
        ...
        if not self.is_visible:
            ...
            # Вставляем панель в родительский контейнер
...
class VideoPlayerThread:
    """Поток для воспроизведения одного видео"""
            ...
            while self.is_playing:
                try:
                    ...
                    if frame.shape[0] != self.video_data['height'] or frame.shape[1] != self.video_data['width']:
                        frame = cv2.resize(frame, (self.video_data['width'], self.video_data['height']), 
...
class VideoGridPlayer:
    def __init__(self, root):
        ...
        # Создаем Canvas для видео (сначала создаем, чтобы потом ссылаться)
        ...
        self.canvas.master = canvas_container  # Сохраняем ссылку
        
        # Создаем панель настроек (после canvas)
        self.settings_panel = SettingsPanel(self.main_container, self.settings, self)
        
        # Создаем содержимое панели управления
    ...
    def create_control_panel_content(self, parent):
        ...
        self.start_btn = tk.Button(button_frame, text="▶ Старт (Пробел)", 
                                  command=self.start_playback, **btn_style)
        self.start_btn.pack(side=tk.LEFT, padx=4)
        
        self.stop_btn = tk.Button(button_frame, text="⏹ Стоп (S)", 
                                 command=self.stop_playback, **btn_style)
        self.stop_btn.pack(side=tk.LEFT, padx=4)
        
        self.settings_btn = tk.Button(button_frame, text="⚙️ Настройки (F1)", 
                                      command=self.settings_panel.toggle, **btn_style)
        self.settings_btn.pack(side=tk.LEFT, padx=4)
        
        self.hide_btn = tk.Button(button_frame, text="🔽 Скрыть панель (H)", 
                                 command=self.toggle_panel, **btn_style)
        self.hide_btn.pack(side=tk.LEFT, padx=4)
        
        self.info_label = tk.Label(button_frame, text="Нет загруженных видео", 
                                  bg='#1c1c1c', fg='#8c8c8c', font=('Segoe UI', 10))
        ...
        self.fps_slider = ttk.Scale(fps_frame, from_=1, to=60, orient=tk.HORIZONTAL, 
                                   variable=self.fps_var, length=150)
        self.fps_slider.pack(side=tk.LEFT, padx=8)
        self.fps_label = tk.Label(fps_frame, text="30", width=4, bg='#1c1c1c', fg='#ffffff',
        ...
        self.cols_slider = ttk.Scale(cols_frame, from_=1, to=8, orient=tk.HORIZONTAL, 
                                    variable=self.cols_var, length=150)
        self.cols_slider.pack(side=tk.LEFT, padx=8)
        self.cols_label = tk.Label(cols_frame, text="3", width=4, bg='#1c1c1c', fg='#ffffff',
                                  font=('Segoe UI', 10, 'bold'))
        ...
        self.show_names_check = tk.Checkbutton(quick_frame, text="📝 Имена файлов", 
        ...
        self.optimize_check = tk.Checkbutton(quick_frame, text="⚡ Оптимизация", 
    ...
    def start_playback(self):
        ...
        for video in self.videos:
            thread = VideoPlayerThread(
                video,
                lambda: self.fps_var.get(),
                lambda: self.optimize_var.get(),
                self.settings
            )
