...
# Отключаем многопоточность FFMPEG для избежания ошибок
...
class VideoPlayerThread:
    """Отдельный поток для воспроизведения одного видео"""
    def __init__(self, video_data, fps_control, optimize_flag):
    ...
    def get_frame(self):
        """Получить последний кадр из очереди"""
        try:
    ...
    def _play_loop(self):
        """Цикл воспроизведения для одного видео"""
        try:
            # Открываем видео в отдельном потоке
            ...
            frame_time = 1.0 / min(fps, self.fps_control())
            last_frame_time = 0
            ...
            while self.is_playing:
                try:
                    ...
                    # Оптимизация размера
                    if self.optimize_flag() and len(self.video_data.get('videos', [])) > 20:
                        # Уменьшаем размер для экономии ресурсов
                        width = self.video_data['width'] // 2
                        height = self.video_data['height'] // 2
            ...
            print(f"Ошибка в потоке видео: {e}")
            
    def _open_video(self):
        """Открытие видео с оптимальными настройками"""
        try:
            # Используем однопоточный режим FFMPEG
            cap = cv2.VideoCapture(self.video_data['path'], cv2.CAP_FFMPEG)
            
            # Отключаем многопоточность
            cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
            
            # Устанавливаем опции FFMPEG для избежания ошибок
            if hasattr(cap, 'set'):
                try:
                    # Устанавливаем опции для однопоточного декодирования
            ...
            return cv2.VideoCapture(self.video_data['path'])
...
class VideoGridPlayer:
    def __init__(self, root):
        ...
        self.backend_options = {
            'SINGLE_THREAD': 'Однопоточный (стабильный)',
            'FFMPEG_SW': 'FFMPEG (CPU)',
        }
        ...
        self.current_backend = tk.StringVar(value='SINGLE_THREAD')
    ...
    def create_control_panel(self, parent):
        ...
        self.backend_combo.set(self.backend_options['SINGLE_THREAD'])
    ...
    def get_video_capture_single_thread(self, path):
        """Создание однопоточного VideoCapture"""
        try:
            # Используем однопоточный режим FFMPEG
    ...
    def start_playback(self):
        ...
        # Создаем потоки для каждого видео
        self.video_threads = []
        
        for video in self.videos:
            # Добавляем ссылку на список видео для оптимизации
            ...
            # Открываем видео в потоке
            if backend == 'SINGLE_THREAD':
                thread.cap = self.get_video_capture_single_thread(video['path'])
            else:
                thread.cap = cv2.VideoCapture(video['path'])
        ...
        # Запускаем таймер обновления canvas
        self._start_update_timer()
    
    def _start_update_timer(self):
        """Запуск таймера обновления canvas"""
...
def main():
    ...
    # Устанавливаем переменные окружения для отключения многопоточности FFMPEG
    os.environ['OPENCV_FFMPEG_THREADS'] = '1'
