...
class SettingsPanel:
    ...
    def create_settings_widgets(self, parent):
        ...
        tk.Label(parent, text="Задержка между открытием видео (мс):", bg='#2c2c2c', fg='#ffffff',
                font=('Segoe UI', 10, 'bold')).grid(row=row, column=0, sticky='w', padx=15, pady=(10,5))
        row += 1
...
class VideoPlayerThread:
    ...
    def _get_video_capture(self, path):
        ...
        # Используем глобальный лок для предотвращения конфликтов FFMPEG
    ...
    def _play_loop(self):
        try:
            # Добавляем задержку для предотвращения конфликтов при открытии
...
class ResizeHandle:
    ...
    def __init__(self, root, app):
        self.root = root
        self.app = app
        self.resize_size = 20  # Увеличил с 10 до 20 для лучшей видимости
        self.resizing = False
...
class VideoGridPlayer:
    ...
    def rearrange_grid(self, keep_playing=False):
        ...
        # Masonry-подобная раскладка с разными высотами
        col_heights = [0] * cols
        ...
        # Обновляем размер scrollable_frame
    ...
    def load_folder(self):
        ...
        # Предварительно читаем пропорции видео
        video_ratios = {}
        for idx, video_path in enumerate(unique_videos):
            try:
                with video_capture_lock:
                    ...
                    cap.release()
                time.sleep(0.02)  # Задержка между открытиями
            except:
                video_ratios[str(video_path)] = 16/9
        
        # Masonry-подобная раскладка
