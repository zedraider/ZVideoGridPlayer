...
class VideoPlayerThread:
    ...
    def stop(self):
        ...
        self.is_playing = False
        # Не ждем поток, просто снимаем флаг
        if self.cap:
            try:
                self.cap.release()
            except:
    ...
    def _play_loop(self):
        try:
            ...
            # Восстанавливаем позицию если была
...
class CustomTitleBar:
    ...
    def setup_resize_handles(self):
        ...
        self.right_edge.place(relx=1.0, rely=0.5, anchor='e', width=s, height=1)
        ...
        self.left_edge.place(relx=0.0, rely=0.5, anchor='w', width=s, height=1)
        ...
        self.bottom_edge.place(relx=0.5, rely=1.0, anchor='s', width=1, height=s)
        ...
        self.top_edge.place(relx=0.5, rely=0.0, anchor='n', width=1, height=s)
...
class VideoGridPlayer:
    ...
    def resume_playback_all(self):
        """Возобновление всех видео"""
        self.is_paused = False
        for thread in self.video_threads:
            thread.resume()
        
    def pause_playback(self):
        """Приостановка для перетаскивания окна"""
        for thread in self.video_threads:
            thread.pause()
