...
class VideoGridPlayer:
    ...
    def rearrange_grid(self, keep_playing=False):
        ...
        # Сохраняем пропорции видео если они уже известны
        video_ratios = {}
        for video in self.videos:
            if 'aspect_ratio' in video and video['aspect_ratio'] > 0:
                video_ratios[video['path']] = video['aspect_ratio']
        ...
        for i, video_data in enumerate(current_videos):
            ...
            # Используем сохраненную пропорцию или дефолтную
            aspect_ratio = video_ratios.get(video_data['path'], 16/9)
            video_height = int(video_width / aspect_ratio) if aspect_ratio > 0 else int(video_width * 0.75)
    ...
    def load_folder(self):
        ...
        for i, video_path in enumerate(unique_videos):
            ...
            # Используем реальную пропорцию видео
