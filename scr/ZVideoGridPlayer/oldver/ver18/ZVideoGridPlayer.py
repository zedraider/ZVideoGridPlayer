...
class SettingsPanel:
    ...
    def create_settings_widgets(self, parent):
        ...
        tk.Label(parent, text="Максимум видео:", bg='#2c2c2c', fg='#ffffff',
                font=('Segoe UI', 10, 'bold')).grid(row=row, column=0, sticky='w', padx=15, pady=(10,5))
        row += 1
        
        max_videos_frame = tk.Frame(parent, bg='#2c2c2c')
        max_videos_frame.grid(row=row, column=0, sticky='ew', padx=15, pady=5)
        
        self.max_videos = tk.IntVar(value=self.settings.get('max_videos', 64))
        ttk.Scale(max_videos_frame, from_=1, to=256, orient=tk.HORIZONTAL, 
                  variable=self.max_videos, length=250).pack(side=tk.LEFT)
        self.max_videos_entry = tk.Entry(max_videos_frame, textvariable=self.max_videos, 
                                         width=6, bg='#3c3c3c', fg='white',
                                         font=('Segoe UI', 10))
        self.max_videos_entry.pack(side=tk.LEFT, padx=10)
        self.max_videos.trace('w', lambda *a: self.max_videos_entry.delete(0, tk.END))
        self.max_videos_entry.bind('<Return>', lambda e: self.update_max_videos())
        row += 1
    ...
    def update_max_videos(self):
        """Обновление максимального количества видео из поля ввода"""
        try:
            val = int(self.max_videos_entry.get())
            if 1 <= val <= 256:
                self.max_videos.set(val)
        except:
            pass
...
class CustomTitleBar:
    ...
    def setup_resize_handles(self):
        """Создание всех областей для растягивания окна"""
        s = self.resize_size
        
        # Углы
        self.bottom_right = tk.Frame(self.root, bg='#1c1c1c', cursor='size_nw_se')
        self.bottom_right.place(relx=1.0, rely=1.0, anchor='se', width=s, height=s)
        ...
        self.bottom_left.place(relx=0.0, rely=1.0, anchor='sw', width=s, height=s)
        ...
        self.top_right.place(relx=1.0, rely=0.0, anchor='ne', width=s, height=s)
        ...
        self.top_left.place(relx=0.0, rely=0.0, anchor='nw', width=s, height=s)
        ...
        self.right_edge.place(relx=1.0, rely=0.5, anchor='e', width=s, height=60)
        ...
        self.left_edge.place(relx=0.0, rely=0.5, anchor='w', width=s, height=60)
        ...
        self.bottom_edge.place(relx=0.5, rely=1.0, anchor='s', width=60, height=s)
        ...
        self.top_edge.place(relx=0.5, rely=0.0, anchor='n', width=60, height=s)
    ...
    def resize_bottom_left(self, event):
        ...
        width = max(800, self.root.winfo_width() + (root_x - x))
    ...
    def resize_top_right(self, event):
        ...
        height = max(600, self.root.winfo_height() + (root_y - y))
...
class VideoGridPlayer:
    ...
    def create_control_panel_content(self, parent):
        ...
        self.cols_slider = ttk.Scale(cols_frame, from_=1, to=16, orient=tk.HORIZONTAL, variable=self.cols_var, length=150)
    ...
    def load_folder(self):
        ...
        for video_path in video_files:
            if video_path.name not in seen_names and len(unique_videos) < max_videos:
        ...
        if len(video_files) > max_videos:
            messagebox.showinfo("Информация", f"Найдено {len(video_files)} видео.\nЗагружено только {max_videos} (максимум в настройках)")
        self.clear_videos()
        self.videos = []
