...
class CustomTitleBar:
    ...
    def setup_resize_handles(self):
        ...
        # Создаем невидимые области для изменения размера
        self.resize_handles = {}
        
        # Размер области для изменения размера (в пикселях)
        resize_size = 6
    ...
    def resize_bottom_right(self, event):
        """Изменение размера из нижнего правого угла"""
        x = self.root.winfo_pointerx() - self.root.winfo_rootx()
        y = self.root.winfo_pointery() - self.root.winfo_rooty()
        if x > 200 and y > 100:
            self.root.geometry(f"{max(800, x)}x{max(600, y)}")
            self.app.update_grid_size()
            
    def resize_bottom_left(self, event):
        """Изменение размера из нижнего левого угла"""
    ...
    def resize_right(self, event):
        """Изменение размера по правому краю"""
    ...
    def resize_bottom(self, event):
        """Изменение размера по нижнему краю"""
    ...
    def minimize_window(self, event):
        """Минимизация окна с остановкой воспроизведения"""
        # Останавливаем воспроизведение перед минимизацией
        self.app.stop_playback()
        
        try:
            # Временно отключаем override-redirect для минимизации
            self.root.overrideredirect(False)
            self.root.iconify()
            # После минимизации включаем обратно при восстановлении
            self.root.after(100, self.restore_override)
        except:
            pass
    
    def restore_override(self):
        """Восстановление override-redirect после восстановления окна"""
        try:
            if self.root.winfo_viewable():
                self.root.overrideredirect(True)
                # Обновляем размеры сетки после восстановления
                self.app.update_grid_size()
        except:
            pass
...
class VideoGridPlayer:
    def __init__(self, root):
        ...
        # Задержка для обновления размеров после загрузки
        self.after_id = None
    ...
    def rearrange_grid(self, keep_playing=False):
        ...
        cols = self.cols
        # Получаем реальную ширину canvas
        canvas_width = self.canvas.winfo_width() - 20
        if canvas_width < 100:
            canvas_width = 1200
        
        # Автоматический расчет размера видео
    ...
    def load_folder(self):
        ...
        # Убираем всплывающее сообщение - просто обновляем статус
        # messagebox больше не показываем
    ...
    def start_playback(self):
        ...
        for video in self.videos:
            if video['cap']:
                video['cap'].release()
            try:
                video['cap'] = cv2.VideoCapture(video['path'], cv2.CAP_FFMPEG)  # Используем FFMPEG для лучшей стабильности
                ...
                print(f"Ошибка открытия видео {video['name']}: {e}")
                video['valid'] = False
                continue
        ...
        while self.is_playing:
            ...
            for idx, video in enumerate(self.videos):
                ...
                try:
                    ...
                    print(f"Ошибка при чтении кадра {video['name']}: {e}")
                    continue
